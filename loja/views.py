...
def ver_produto(request, id_produto, id_cor=None):
    ...
    similares = Produto.objects.filter(categoria__id=produto.categoria.id, tipo__id=produto.tipo.id).exclude(id=id_produto)[:4]
...
def finalizar_pagamento(request):
    #collection_id=104545995631&collection_status=approved&payment_id=104545995631&status=approved&external_reference=null&payment_type=account_money&merchant_order_id=29417437905&preference_id=2302897061-1a3995b4-a33b-45dc-83a4-d2016c5f2f5c&site_id=MLB&processing_mode=aggregator&merchant_account_id=null HTTP/1.1" 302 0
...
def meus_pedidos(request):
    ...
    print("Cliente associado:", getattr(request.user, 'cliente', 'Sem cliente'))
...
def gerenciar_loja(request):
    if request.user.groups.filter(name="equipe").exists():
        ...
        qtde_pedido = len(pedidos_finalizados)
        ...
        context = {"qtde_pedidos": qtde_pedido, "qtde_produtos": qtde_produtos, "faturamento": faturamento}
